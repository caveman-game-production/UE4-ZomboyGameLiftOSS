...
void FGameLiftServerSDKModule::StartupModule()
{
    ...
    #if PLATFORM_64BITS
        #if WITH_GAMELIFT
            FString BaseDir = IPluginManager::Get().FindPlugin("OnlineSubsystemZomboy")->GetBaseDir();
...
TGameLiftOutcome<Aws::GameLift::Server::Model::DescribePlayerSessionsResult, FGameLiftError> FGameLiftServerSDKModule::DescribePlayerSessions(const FString& gamesessionId)
{
#if WITH_GAMELIFT
	...
	if (outcome.IsSuccess()) {
		return TGameLiftOutcome<Aws::GameLift::Server::Model::DescribePlayerSessionsResult, FGameLiftError>(outcome.GetResult());
	}
	else {
		return TGameLiftOutcome<Aws::GameLift::Server::Model::DescribePlayerSessionsResult, FGameLiftError>(FGameLiftError(outcome.GetError()));
	}
#else
	return TGameLiftOutcome<Aws::GameLift::Server::Model::DescribePlayerSessionsResult, FGameLiftError>(FGameLiftError());
#endif
}
