...
FOnlineSessionInfoZomboy::FOnlineSessionInfoZomboy() :
	HostAddr(NULL),
	SessionId(TEXT("INVALID"))
{
}

void FOnlineSessionInfoZomboy::Init(const FOnlineSubsystemZomboy& Subsystem)
{
...
/**
 *	Async task for ending a Zomboy online session
 */
class FOnlineAsyncTaskZomboyEndSession : public FOnlineAsyncTaskBasic<FOnlineSubsystemZomboy>
{
private:
...
public:
	FOnlineAsyncTaskZomboyEndSession(class FOnlineSubsystemZomboy* InSubsystem, FName InSessionName) :
	...
	~FOnlineAsyncTaskZomboyEndSession()
	{
	...
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskZomboyEndSession bWasSuccessful: %d SessionName: %s"), bWasSuccessful, *SessionName.ToString());
	}
...
/**
 *	Async task for destroying a Zomboy online session
 */
class FOnlineAsyncTaskZomboyDestroySession : public FOnlineAsyncTaskBasic<FOnlineSubsystemZomboy>
{
private:
	/** Name of session ending */
	FName SessionName;
...
public:
	FOnlineAsyncTaskZomboyDestroySession(class FOnlineSubsystemZomboy* InSubsystem, FName InSessionName) :
		FOnlineAsyncTaskBasic(InSubsystem),
		SessionName(InSessionName)
	{
	}

	~FOnlineAsyncTaskZomboyDestroySession()
	{
	}
	...
	virtual FString ToString() const override
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskZomboyDestroySession bWasSuccessful: %d SessionName: %s"), bWasSuccessful, *SessionName.ToString());
	}
...
bool FOnlineSessionZomboy::CreateSession(int32 HostingPlayerNum, FName SessionName, const FOnlineSessionSettings& NewSessionSettings)
{
	...
	if (Session == NULL)
	{
		...
		IOnlineIdentityPtr Identity = ZomboySubsystem->GetIdentityInterface();
		if (Identity.IsValid())
		{
			...
			if (Session->OwningUserId.IsValid())
			{
				if (!NewSessionSettings.bIsLANMatch)
				{
					...
					else
					{
						...
						Result = CreateGameLiftSession(HostingPlayerNum, Session);
#else
...
bool FOnlineSessionZomboy::CreateSession(const FUniqueNetId& HostingPlayerId, FName SessionName, const FOnlineSessionSettings& NewSessionSettings)
{
	// todo: use proper	HostingPlayerId
	return CreateSession(0, SessionName, NewSessionSettings);
}

uint32 FOnlineSessionZomboy::CreateGameLiftSession(int32 HostingPlayerNum, FNamedOnlineSession* Session)
{
	uint32 Result = E_FAIL;
	...
	if (Session)
	{
		...
		if (Identity.IsValid())
		{
			TSharedPtr<const FUniqueNetId> LocalPlayerId = Identity->GetUniquePlayerId(HostingPlayerNum);

			if (LocalPlayerId.IsValid() && LocalPlayerId->IsValid())
			{
				Session->OwningUserId = LocalPlayerId;

				TArray<TSharedRef<const FUniqueNetId>> PlayerIds;
				PlayerIds.Add(LocalPlayerId.ToSharedRef());

				/** Generate the proper lobby type from our session settings */
				FOnlineAsyncTaskCreateGameliftSession* NewTask = new FOnlineAsyncTaskCreateGameliftSession(ZomboySubsystem, Session->SessionName, PlayerIds);
...
bool FOnlineSessionZomboy::NeedsToAdvertise()
{
...
bool FOnlineSessionZomboy::NeedsToAdvertise( FNamedOnlineSession& Session )
{
...
bool FOnlineSessionZomboy::IsSessionJoinable(const FNamedOnlineSession& Session) const
{
...
uint32 FOnlineSessionZomboy::CreateLANSession(int32 HostingPlayerNum, class FNamedOnlineSession* Session)
{
	...
	NewSessionInfo->Init(*ZomboySubsystem);
	Session->SessionInfo = MakeShareable(NewSessionInfo);

	return UpdateLANStatus();
}

uint32 FOnlineSessionZomboy::UpdateLANStatus()
{
	...
	if ( NeedsToAdvertise() )
	{
		...
		{
			FOnValidQueryPacketDelegate QueryPacketDelegate = FOnValidQueryPacketDelegate::CreateRaw(this, &FOnlineSessionZomboy::OnValidQueryPacketReceived);
...
bool FOnlineSessionZomboy::StartSession(FName SessionName)
{
	uint32 Result = E_FAIL;
...
bool FOnlineSessionZomboy::UpdateSession(FName SessionName, FOnlineSessionSettings& UpdatedSessionSettings, bool bShouldRefreshOnlineData)
{
...
bool FOnlineSessionZomboy::EndSession(FName SessionName)
{
	uint32 Result = E_FAIL;
...
bool FOnlineSessionZomboy::DestroySession(FName SessionName, const FOnDestroySessionCompleteDelegate& CompletionDelegate)
{
	uint32 Result = E_FAIL;
	...
	if (Session)
	{
		if (Session->SessionState != EOnlineSessionState::Destroying)
		{
			...
			{
#if !WITH_GAMELIFT_SERVER
				Result = DestroyGameLiftSession(Session);
#else
			...
			IOnlineVoicePtr VoiceInt = ZomboySubsystem->GetVoiceInterface();
			if (VoiceInt.IsValid())
			{
				if (!ZomboySubsystem->IsDedicated())
				{
					// Stop local talkers
...
uint32 FOnlineSessionZomboy::DestroyGameLiftSession(FNamedOnlineSession* Session)
{
	Session->SessionState = EOnlineSessionState::Destroying;
	if (Session->SessionInfo.IsValid())
	{
		FOnlineAsyncTaskZomboyDestroySession* NewTask = new FOnlineAsyncTaskZomboyDestroySession(ZomboySubsystem, Session->SessionName);
		ZomboySubsystem->QueueAsyncTask(NewTask);
	}
...
bool FOnlineSessionZomboy::IsPlayerInSession(FName SessionName, const FUniqueNetId& UniqueId)
{
	return IsPlayerInSessionImpl(this, SessionName, UniqueId);
}

bool FOnlineSessionZomboy::StartMatchmaking(const TArray< TSharedRef<const FUniqueNetId> >& LocalPlayers, FName SessionName, const FOnlineSessionSettings& NewSessionSettings, TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
	uint32 Result = E_FAIL;
...
bool FOnlineSessionZomboy::CancelMatchmaking(int32 SearchingPlayerNum, FName SessionName)
{
	uint32 Result = E_FAIL;
	...
	if (Session)
	{
		FOnlineAsyncTaskStopGameliftMatchmaking* NewTask = new FOnlineAsyncTaskStopGameliftMatchmaking(ZomboySubsystem, SessionName);
...
bool FOnlineSessionZomboy::CancelMatchmaking(const FUniqueNetId& SearchingPlayerId, FName SessionName)
{
	UE_LOG(LogOnline, Warning, TEXT("CancelMatchmaking is not supported on this platform. Use CancelFindSessions."));
	TriggerOnCancelMatchmakingCompleteDelegates(SessionName, false);
	return false;
...
bool FOnlineSessionZomboy::FindSessions(int32 SearchingPlayerNum, const TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
	...
	if (!CurrentSessionSearch.IsValid() && SearchSettings->SearchState != EOnlineAsyncTaskState::InProgress)
	{
		...
		{
			Return = FindGameLiftSession(SearchSettings);
		}
		else
		{
...
bool FOnlineSessionZomboy::FindSessions(const FUniqueNetId& SearchingPlayerId, const TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
...
bool FOnlineSessionZomboy::FindSessionById(const FUniqueNetId& SearchingUserId, const FUniqueNetId& SessionId, const FUniqueNetId& FriendId, const FOnSingleSessionResultCompleteDelegate& CompletionDelegates)
{
...
uint32 FOnlineSessionZomboy::FindGameLiftSession(const TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
#if WITH_GAMELIFTCLIENTSDK

	FOnlineAsyncTaskSearchGameliftSession* NewTask = new FOnlineAsyncTaskSearchGameliftSession(ZomboySubsystem, SearchSettings);
	ZomboySubsystem->QueueAsyncTask(NewTask);
...
uint32 FOnlineSessionZomboy::FindLANSession()
{
	...
	FOnValidResponsePacketDelegate ResponseDelegate = FOnValidResponsePacketDelegate::CreateRaw(this, &FOnlineSessionZomboy::OnValidResponsePacketReceived);
	FOnSearchingTimeoutDelegate TimeoutDelegate = FOnSearchingTimeoutDelegate::CreateRaw(this, &FOnlineSessionZomboy::OnLANSearchTimeout);

	FNboSerializeToBufferZomboy Packet(LAN_BEACON_MAX_PACKET_SIZE);
...
bool FOnlineSessionZomboy::CancelFindSessions()
{
	uint32 Return = E_FAIL;
...
bool FOnlineSessionZomboy::JoinSession(int32 PlayerNum, FName SessionName, const FOnlineSessionSearchResult& DesiredSession)
{
	uint32 Return = E_FAIL;
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
...
bool FOnlineSessionZomboy::JoinSession(const FUniqueNetId& PlayerId, FName SessionName, const FOnlineSessionSearchResult& DesiredSession)
{
	// Assuming player 0 should be OK here
	return JoinSession(0, SessionName, DesiredSession);
}

uint32 FOnlineSessionZomboy::JoinGameLiftSession(int32 PlayerNum, FNamedOnlineSession* Session, const FOnlineSession* SearchSession)
{
	uint32 Result = E_FAIL;
	
	if (SearchSession && SearchSession->SessionInfo.IsValid())
	{
		FOnlineSessionInfoZomboy* ZomboySessionInfo = (FOnlineSessionInfoZomboy*)SearchSession->SessionInfo.Get();
		if (ZomboySessionInfo->SessionId.IsValid())
		{
#if WITH_GAMELIFTCLIENTSDK

			// The settings found on the search object will be duplicated again when we enter the lobby, possibly updated
			FOnlineAsyncTaskJoinGameliftSession* NewTask = new FOnlineAsyncTaskJoinGameliftSession(ZomboySubsystem, Session->SessionName, ZomboySessionInfo->SessionId, ZomboySessionInfo->SessionRegion);
			ZomboySubsystem->QueueAsyncTask(NewTask);
...
bool FOnlineSessionZomboy::FindFriendSession(int32 LocalUserNum, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Zomboy subsystem
...
bool FOnlineSessionZomboy::FindFriendSession(const FUniqueNetId& LocalUserId, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Zomboy subsystem
	TArray<FOnlineSessionSearchResult> EmptySearchResult;
...
bool FOnlineSessionZomboy::FindFriendSession(const FUniqueNetId& LocalUserId, const TArray<TSharedRef<const FUniqueNetId>>& FriendList)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Zomboy subsystem
	TArray<FOnlineSessionSearchResult> EmptySearchResult;
	TriggerOnFindFriendSessionCompleteDelegates(0, false, EmptySearchResult);
...
bool FOnlineSessionZomboy::SendSessionInviteToFriend(int32 LocalUserNum, FName SessionName, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Zomboy subsystem
	return false;
};

bool FOnlineSessionZomboy::SendSessionInviteToFriend(const FUniqueNetId& LocalUserId, FName SessionName, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Zomboy subsystem
	return false;
}

bool FOnlineSessionZomboy::SendSessionInviteToFriends(int32 LocalUserNum, FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Friends)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Zomboy subsystem
	return false;
};

bool FOnlineSessionZomboy::SendSessionInviteToFriends(const FUniqueNetId& LocalUserId, FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Friends)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Zomboy subsystem
	return false;
}

uint32 FOnlineSessionZomboy::JoinLANSession(int32 PlayerNum, FNamedOnlineSession* Session, const FOnlineSession* SearchSession)
{
...
bool FOnlineSessionZomboy::PingSearchResults(const FOnlineSessionSearchResult& SearchResult)
{
	return false;
}

/** Get a resolved connection string from a session info */
static bool GetConnectStringFromSessionInfo(TSharedPtr<FOnlineSessionInfoZomboy>& SessionInfo, FString& ConnectInfo, int32 PortOverride=0)
{
	bool bSuccess = false;
	if (SessionInfo.IsValid())
	{
		if (SessionInfo->HostAddr.IsValid() && SessionInfo->HostAddr->IsValid())
		{
			...
			if (!SessionInfo->GameLiftPlayerId.IsEmpty())
			{
				ConnectInfo += FString::Printf(TEXT("?%s="), GAMELIFT_URL_PLAYER_SESSION_ID);
				ConnectInfo += SessionInfo->GameLiftPlayerId;
			}

			ConnectInfo += FString::Printf(TEXT("?%s="), GAMELIFT_URL_PLAYER_REGION_KEY);
			ConnectInfo += GetRegionString(SessionInfo->GameLiftPlayerRegion);

			ConnectInfo += FString::Printf(TEXT("?%s="), GAMELIFT_URL_PLAYER_JOINTYPE_KEY);
			ConnectInfo += FString::FromInt(SessionInfo->bFromMatchmaking ? 1 : 0);

			if (SessionInfo->bIsHost)
			{
				ConnectInfo += TEXT("?IsHost");
			}
...
bool FOnlineSessionZomboy::GetResolvedConnectString(FName SessionName, FString& ConnectInfo, FName PortType)
{
	bool bSuccess = false;
	// Find the session
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
...
bool FOnlineSessionZomboy::GetResolvedConnectString(const FOnlineSessionSearchResult& SearchResult, FName PortType, FString& ConnectInfo)
{
	bool bSuccess = false;
	if (SearchResult.Session.SessionInfo.IsValid())
	{
		TSharedPtr<FOnlineSessionInfoZomboy> SessionInfo = StaticCastSharedPtr<FOnlineSessionInfoZomboy>(SearchResult.Session.SessionInfo);
...
FOnlineSessionSettings* FOnlineSessionZomboy::GetSessionSettings(FName SessionName)
{
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
	if (Session)
	{
...
void FOnlineSessionZomboy::RegisterLocalPlayers(FNamedOnlineSession* Session)
{
	if (!ZomboySubsystem->IsDedicated())
	{
		IOnlineVoicePtr VoiceInt = ZomboySubsystem->GetVoiceInterface();
...
void FOnlineSessionZomboy::RegisterVoice(const FUniqueNetId& PlayerId)
{
	IOnlineVoicePtr VoiceInt = ZomboySubsystem->GetVoiceInterface();
	if (VoiceInt.IsValid())
	{
		if (!ZomboySubsystem->IsLocalPlayer(PlayerId))
		{
...
void FOnlineSessionZomboy::UnregisterVoice(const FUniqueNetId& PlayerId)
{
	IOnlineVoicePtr VoiceInt = ZomboySubsystem->GetVoiceInterface();
	if (VoiceInt.IsValid())
	{
...
bool FOnlineSessionZomboy::RegisterPlayer(FName SessionName, const FUniqueNetId& PlayerId, bool bWasInvited)
{
	TArray< TSharedRef<const FUniqueNetId> > Players;
	//Players.Add(MakeShareable(new FUniqueNetIdString(PlayerId)));
	Players.Add(MakeShareable(new FUniqueNetIdZomboyPlayer(PlayerId)));
	return RegisterPlayers(SessionName, Players, bWasInvited);
}

bool FOnlineSessionZomboy::RegisterPlayers(FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Players, bool bWasInvited)
{
	bool bSuccess = false;
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
	if (Session)
...
bool FOnlineSessionZomboy::UnregisterPlayer(FName SessionName, const FUniqueNetId& PlayerId)
{
	TArray< TSharedRef<const FUniqueNetId> > Players;
	//Players.Add(MakeShareable(new FUniqueNetIdString(PlayerId)));
	Players.Add(MakeShareable(new FUniqueNetIdZomboyPlayer(PlayerId)));
...
bool FOnlineSessionZomboy::UnregisterPlayers(FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Players)
{
	bool bSuccess = true;
...
void FOnlineSessionZomboy::Tick(float DeltaTime)
{
	SCOPE_CYCLE_COUNTER(STAT_Session_Interface);
	TickLanTasks(DeltaTime);
}

void FOnlineSessionZomboy::TickLanTasks(float DeltaTime)
{
	LANSessionManager.Tick(DeltaTime);
}

void FOnlineSessionZomboy::AppendSessionToPacket(FNboSerializeToBufferZomboy& Packet, FOnlineSession* Session)
{
	/** Owner of the session */
      Packet << Session->OwningUserId->ToString()
		<< Session->OwningUserName
		<< Session->NumOpenPrivateConnections
		<< Session->NumOpenPublicConnections;

	// Try to get the actual port the netdriver is using
	SetPortFromNetDriver(*ZomboySubsystem, Session->SessionInfo);

	// Write host info (host addr, session id, and key)
	Packet << *StaticCastSharedPtr<FOnlineSessionInfoZomboy>(Session->SessionInfo);

	// Now append per game settings
	AppendSessionSettingsToPacket(Packet, &Session->SessionSettings);
}

void FOnlineSessionZomboy::AppendSessionSettingsToPacket(FNboSerializeToBufferZomboy& Packet, FOnlineSessionSettings* SessionSettings)
{
#if DEBUG_LAN_BEACON
...
void FOnlineSessionZomboy::OnValidQueryPacketReceived(uint8* PacketData, int32 PacketLength, uint64 ClientNonce)
{
...
void FOnlineSessionZomboy::ReadSessionFromPacket(FNboSerializeFromBufferZomboy& Packet, FOnlineSession* Session)
{
#if DEBUG_LAN_BEACON
	...
	FUniqueNetIdZomboyPlayer* UniqueId = new FUniqueNetIdZomboyPlayer;
	Packet >> *UniqueId
		>> Session->OwningUserName
		>> Session->NumOpenPrivateConnections
		>> Session->NumOpenPublicConnections;
	...
	FOnlineSessionInfoZomboy* ZomboySessionInfo = new FOnlineSessionInfoZomboy();
	ZomboySessionInfo->HostAddr = ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->CreateInternetAddr();
	Packet >> *ZomboySessionInfo;
	Session->SessionInfo = MakeShareable(ZomboySessionInfo); 

	// Read any per object data using the server object
	ReadSettingsFromPacket(Packet, Session->SessionSettings);
}

void FOnlineSessionZomboy::ReadSettingsFromPacket(FNboSerializeFromBufferZomboy& Packet, FOnlineSessionSettings& SessionSettings)
{
#if DEBUG_LAN_BEACON
...
void FOnlineSessionZomboy::OnValidResponsePacketReceived(uint8* PacketData, int32 PacketLength)
{
	...
	if (CurrentSessionSearch.IsValid())
	{
		...
		FNboSerializeFromBufferZomboy Packet(PacketData, PacketLength);
...
uint32 FOnlineSessionZomboy::FinalizeLANSearch()
{
	if (LANSessionManager.GetBeaconState() == ELanBeaconState::Searching)
	{
		LANSessionManager.StopLANSession();
...
void FOnlineSessionZomboy::OnLANSearchTimeout()
{
	FinalizeLANSearch();
...
int32 FOnlineSessionZomboy::GetNumSessions()
{
	FScopeLock ScopeLock(&SessionLock);
	return Sessions.Num();
}

void FOnlineSessionZomboy::DumpSessionState()
{
	FScopeLock ScopeLock(&SessionLock);
...
void FOnlineSessionZomboy::RegisterLocalPlayer(const FUniqueNetId& PlayerId, FName SessionName, const FOnRegisterLocalPlayerCompleteDelegate& Delegate)
{
	Delegate.ExecuteIfBound(PlayerId, EOnJoinSessionCompleteResult::Success);
}

void FOnlineSessionZomboy::UnregisterLocalPlayer(const FUniqueNetId& PlayerId, FName SessionName, const FOnUnregisterLocalPlayerCompleteDelegate& Delegate)
{
	Delegate.ExecuteIfBound(PlayerId, true);
}

void FOnlineSessionZomboy::SetPortFromNetDriver(const FOnlineSubsystemZomboy& Subsystem, const TSharedPtr<FOnlineSessionInfo>& SessionInfo)
{
	auto NetDriverPort = GetPortFromNetDriver(Subsystem.GetInstanceName());
	auto SessionInfoZomboy = StaticCastSharedPtr<FOnlineSessionInfoZomboy>(SessionInfo);
	if (SessionInfoZomboy.IsValid() && SessionInfoZomboy->HostAddr.IsValid())
	{
		SessionInfoZomboy->HostAddr->SetPort(NetDriverPort);
	}
}

bool FOnlineSessionZomboy::IsHost(const FNamedOnlineSession& Session) const
{
	if (ZomboySubsystem->IsDedicated())
	{
		return true;
	...
	IOnlineIdentityPtr IdentityInt = ZomboySubsystem->GetIdentityInterface(); 
