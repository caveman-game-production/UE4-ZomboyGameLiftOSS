...
typedef FOnlineKeyValuePairs<FString, FString> FZomboySessionKeyValuePairs;
 ...
 */
class FOnlineSessionZomboy : public IOnlineSession
{
private:

	/** Reference to the main Zomboy subsystem */
	...
	/** Hidden on purpose */
	FOnlineSessionZomboy() :
		ZomboySubsystem(NULL),
	...
	uint32 CreateGameLiftSession(int32 HostingPlayerNum, FNamedOnlineSession* Session);

	uint32 CreateLANSession(int32 HostingPlayerNum, class FNamedOnlineSession* Session);

	uint32 FindGameLiftSession(const TSharedRef<FOnlineSessionSearch>& SearchSettings);
	 ...
	 */
	void AppendSessionToPacket(class FNboSerializeToBufferZomboy& Packet, class FOnlineSession* Session);
	 ...
	 */
	void AppendSessionSettingsToPacket(class FNboSerializeToBufferZomboy& Packet, FOnlineSessionSettings* SessionSettings);
	 ...
	 */
	void ReadSessionFromPacket(class FNboSerializeFromBufferZomboy& Packet, class FOnlineSession* Session);
	 ...
	 * @param SessionSettings the session settings to copy the data to
	 */
	void ReadSettingsFromPacket(class FNboSerializeFromBufferZomboy& Packet, FOnlineSessionSettings& SessionSettings);
	 ...
	 */
	static void SetPortFromNetDriver(const FOnlineSubsystemZomboy& Subsystem, const TSharedPtr<FOnlineSessionInfo>& SessionInfo);
...
PACKAGE_SCOPE:
	...
	/** Current gamelift matchmaking config name. */
	FString GameLiftMatchmakingConfigName;

	FOnlineSessionZomboy(class FOnlineSubsystemZomboy* InSubsystem) :
...
typedef TSharedPtr<FOnlineSessionZomboy, ESPMode::ThreadSafe> FOnlineSessionNullPtr;
